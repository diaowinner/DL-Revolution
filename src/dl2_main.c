...
* dReader 20200308                                                *
*******************************************************************
*    Copyright 2013 Anderain Lovelace, 2020 diaowinner            *
*    anderain.develop@gmail.com        diaowinner@qq.com          *
*******************************************************************/
...
const char * MAIN_MENU_TEXT[] = {" 打开闪存文件 "," 打开SD卡文件 "," 更换阅读字体 "," 关于程序 "," 退出 "};
const char * READ_MENU_TEXT[] = {" 跳页 "," 读取书签 "," 保存书签 "," 退出 "};
...
void start_read ()
{
	...
	while(1)
	{
		...
		else if (key==KEY_CTRL_EXIT)
		{
			...
			menu_index = show_menu (0,4,READ_MENU_TEXT,32,16);
...
int AddIn_main(int isAppli, unsigned short OptionNum)
{
	...
	{
		all_clr();
		PrintXY(0,0,"   DLiterature 2.1",0);
		menu_index = show_menu (menu_index,5,MAIN_MENU_TEXT,24,16);
		...
		{
			r = file_view(0,"dlf",temp_file_name);
			if (r==1)
			{
				close_font(usr_font);
				usr_font = open_font(temp_file_name);
			}
		}
		else if (menu_index==3)
		{
			// about
			show_dialog(" 关于 ","dlreadDLv2.1,原作:直径君,此程序以MPL开源,使用请保留此信息.",0,DLG_BTN_OK);
		}
